...
class MyCalendar:
    ...
    def bianry_search(self, start, end):
        ...
        lower_bound = bisect.bisect_left(
            [self.time_ranges[i][0] for i in range(len(self.time_ranges))], start
        )
        ...
        if (
            lower_bound < len(self.time_ranges)
            and self.time_ranges[lower_bound][0] < end
        ):
            return False
...
class MyCalendar:
    ...
    def book(self, start: int, end: int) -> bool:
        prev_event = (
            self.calendar.peekitem(self.calendar.bisect_left(start) - 1)
            if self.calendar
            else None
        )
        next_event = (
            self.calendar.peekitem(self.calendar.bisect_right(start))
            if self.calendar
            else None
        )

        if prev_event and prev_event[1] > start:
            return False

        if next_event and next_event[0] < end:
            return False
