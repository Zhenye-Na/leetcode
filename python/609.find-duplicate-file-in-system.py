...
class Solution:
    def findDuplicate(self, paths: List[str]) -> List[List[str]]:
        ...
        file_content_hash = defaultdict(list)
        for p in paths:
            path_array = p.split(" ")
            root_dir = path_array[0]

            for i in range(1, len(path_array)):
                filename, file_content = path_array[i].split("(")
                file_content_hash[file_content].append("{}/{}".format(root_dir, filename))

        res = []
        for file_content in file_content_hash:
            dup_files = file_content_hash[file_content]
            if len(dup_files) > 1:
                res.append(file_content_hash[file_content])
